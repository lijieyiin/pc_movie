<template>
    <div>
        ...
        <div class="movie">
            <div :class="i%6==0?'movie_info mvoie_finfo_margin':'movie_info mvoie_oinfo_margin'"  v-for="(item,i) in comMovieList" :key="i">
                <router-link tag="div" :to="{path:'/movieDetail',query:{type:2,movieId:item.movieId}}">
                    <div class="movie_img">
                        <span>预售</span>
                        <img :src=item.img alt="">
                    </div>
                    <div class="movie_title"><p class="p_title">{{item.title}}</p></div>
                    <div class="movie_score">{{item.score}}人想看</div>
                </router-link>
            </div>
        </div>
...
export default {
    ...
    methods: {
        getData(page){
            ...
            var _this = this
            this.$axios.get(this.$store.state.apiUrl + "api/moviesComeList?&limit=" + 24 + "&page=" + page + "&text=" + "")
                .then(function(result) {
                _this.comMovieList = result.data.data;
